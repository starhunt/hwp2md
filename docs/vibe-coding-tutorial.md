...
**프롬프트:**
```
스테이지1 문서 파싱에 업스테이지 도쿠먼트 파서를 사용하는 옵션을 추가하고자해.
먼저 이에 대한 조사를 퍼플렉시티로 조사해줘.
```

**후속:**
```
아냐. 업스테이지 도큐먼트 파서는 HWP지원해.
한국 국내에서는 HWP지원 안하면 공공기관에 판매가 안돼. 다시 조사해봐.
한국어로 쿼리를 해야 해.
```

**패턴:** 조사 결과 검증 및 재조사 요청
...
```
이슈 생성하고 구현 진행시켜.
기본은 내장 파서 쓰고 환경변수 설정하면 업스테이지 도큐먼트 파서를 사용하는것으로 해줘.
```
...
**프롬프트:**
```
LLM 프로바이더에 업스테이지도 추가하고 각 모델들이 bedrock이나 로컬 서버와 같은
...
```
스테이지2 결과도 claude, gpt, gemini, solar 네 종류의 LLM으로 생성한 결과물을 올리고
링크를 걸어서 비교할 수 있게 해줘.
```
...
| 5. 확장 | Upstage 연동, 병합 셀 처리 | "옵션을 추가하고자해" |
